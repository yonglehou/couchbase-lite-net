...
namespace Couchbase
{
	...
	public class CBLiteException : Exception
	{
		...
		public CBLiteException(string detailMessage, int statusCode) : this(detailMessage
			, new CBLStatus(statusCode))
		{
		}
		...
		public CBLiteException(Exception throwable, int statusCode) : base(throwable)
		{
			this.status = new CBLStatus(statusCode);
		}
