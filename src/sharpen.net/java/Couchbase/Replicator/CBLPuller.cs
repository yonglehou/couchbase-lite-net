...
namespace Couchbase.Replicator
{
	public class CBLPuller : CBLReplicator, CBLChangeTrackerClient
	{
		...
		protected internal IList<CBLRevisionInternal> revsToPull;
		...
		public override void BeginReplicating()
		{
			if (downloadsToInsert == null)
			{
				int capacity = 200;
				int delay = 1000;
				downloadsToInsert = new CBLBatcher<IList<object>>(workExecutor, capacity, delay, 
					new _CBLBatchProcessor_58(this));
			}
		...
		private sealed class _CBLBatchProcessor_58 : CBLBatchProcessor<IList<object>>
		{
			public _CBLBatchProcessor_58(CBLPuller _enclosing)
			{
		...
		public override void ProcessInbox(CBLRevisionList inbox)
		{
			...
			lock (this)
			{
				if (revsToPull == null)
				{
					revsToPull = new AList<CBLRevisionInternal>(200);
				}
		...
		public virtual void PullRemoteRevisions()
		{
			...
			IList<CBLRevisionInternal> workToStartNow = new AList<CBLRevisionInternal>();
			lock (this)
			{
			...
			foreach (CBLRevisionInternal work_1 in workToStartNow)
			{
		...
		/// </remarks>
		public virtual void PullRemoteRevision(CBLRevisionInternal rev)
		{
			AsyncTaskStarted();
			++httpConnectionCount;
			...
			SendAsyncMultipartDownloaderRequest("GET", pathInside, null, db, new _CBLRemoteRequestCompletionBlock_268
				(this, rev, pathInside));
		}

		private sealed class _CBLRemoteRequestCompletionBlock_268 : CBLRemoteRequestCompletionBlock
		{
			public _CBLRemoteRequestCompletionBlock_268(CBLPuller _enclosing, CBLRevisionInternal
				 rev, string pathInside)
			{
				this._enclosing = _enclosing;
				this.rev = rev;
			...
			private readonly CBLRevisionInternal rev;
			...
			try
			{
				foreach (IList<object> revAndHistory in revs)
				{
					...
					catch (CBLiteException e)
					{
						...
						{
							Log.W(CBLDatabase.Tag, this + " failed to write " + rev + ": status=" + e.GetCBLStatus
								().GetCode());
		...
		private sealed class _IComparer_315 : IComparer<IList<object>>
		{
			public _IComparer_315()
			{
			}

			public int Compare(IList<object> list1, IList<object> list2)
			{
				CBLRevisionInternal reva = (CBLRevisionInternal)list1[0];
				CBLRevisionInternal revb = (CBLRevisionInternal)list2[0];
				return CBLMisc.TDSequenceCompare(reva.GetSequence(), revb.GetSequence());
			}
		}

		internal virtual IList<string> KnownCurrentRevIDs(CBLRevisionInternal rev)
		{
			if (db != null)
			{
	...
	/// 	</remarks>
	internal class TDPulledRevision : CBLRevisionInternal
	{
