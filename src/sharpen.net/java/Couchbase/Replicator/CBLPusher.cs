...
namespace Couchbase.Replicator
{
	public class CBLPusher : CBLReplicator, Observer
	{
		...
		public override void ProcessInbox(CBLRevisionList inbox)
		{
			...
			foreach (CBLRevisionInternal rev in inbox)
			{
		...
		private sealed class _CBLRemoteRequestCompletionBlock_172 : CBLRemoteRequestCompletionBlock
		{
			...
			public void OnCompletion(object response, Exception e)
			{
				...
				else
				{
					if (results.Count != 0)
					{
						...
						this._enclosing.SendAsyncRequest("POST", "/_bulk_docs", bulkDocsBody, new _CBLRemoteRequestCompletionBlock_239
			...
			private sealed class _CBLRemoteRequestCompletionBlock_239 : CBLRemoteRequestCompletionBlock
			{
				public _CBLRemoteRequestCompletionBlock_239(_CBLRemoteRequestCompletionBlock_172 
		...
		private bool UploadMultipartRevision(CBLRevisionInternal revision)
		{
			...
			AsyncTaskStarted();
			SendAsyncMultipartRequest("PUT", path, multiPart, new _CBLRemoteRequestCompletionBlock_325
				(this));
			return true;
		}
		...
		{
			public _CBLRemoteRequestCompletionBlock_325(CBLPusher _enclosing)
			{
				this._enclosing = _enclosing;
			}
