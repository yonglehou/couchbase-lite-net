...
namespace Couchbase.Replicator
{
	public abstract class CBLReplicator : Observable
	{
		...
		protected internal CBLBatcher<CBLRevisionInternal> batcher;
		...
		public CBLReplicator(CBLDatabase db, Uri remote, bool continuous, HttpClientFactory
			 clientFacotry, ScheduledExecutorService workExecutor)
		{
			this.db = db;
			...
			if (remote.GetQuery() != null && !remote.GetQuery().IsEmpty())
			{
				...
				string personaAssertion = uri.GetQueryParameter(CBLPersonaAuthorizer.QueryParameter
					);
				...
				string facebookAccessToken = uri.GetQueryParameter(CBLFacebookAuthorizer.QueryParameter
					);
				if (facebookAccessToken != null && !facebookAccessToken.IsEmpty())
				{
					string email = uri.GetQueryParameter(CBLFacebookAuthorizer.QueryParameterEmail);
			...
			batcher = new CBLBatcher<CBLRevisionInternal>(workExecutor, InboxCapacity, ProcessorDelay
				, new _CBLBatchProcessor_113(this));
			this.clientFacotry = clientFacotry != null ? clientFacotry : new CBLHttpClientFactory
				();
		}

		private sealed class _CBLBatchProcessor_113 : CBLBatchProcessor<CBLRevisionInternal
			>
		{
			...
			public void Process(IList<CBLRevisionInternal> inbox)
			{
		...
		public virtual void AddToInbox(CBLRevisionInternal rev)
		{
			if (batcher.Count() == 0)
			{
				active = true;
		...
		public virtual void SendAsyncRequest(string method, Uri url, object body, CBLRemoteRequestCompletionBlock
			 onCompletion)
		{
			CBLRemoteRequest request = new CBLRemoteRequest(workExecutor, clientFacotry, method
			...
			, object body, CBLDatabase db, CBLRemoteRequestCompletionBlock onCompletion)
		{
			try
			{
				...
				CBLRemoteMultipartDownloaderRequest request = new CBLRemoteMultipartDownloaderRequest
					(workExecutor, clientFacotry, method, url, body, db, onCompletion);
				remoteRequestExecutor.Execute(request);
			}
			catch (UriFormatException e)
			{
			...
			MultipartEntity multiPartEntity, CBLRemoteRequestCompletionBlock onCompletion)
		{
			...
			CBLRemoteMultipartRequest request = new CBLRemoteMultipartRequest(workExecutor, clientFacotry
		...
		public virtual void FetchRemoteCheckpointDoc()
		{
			...
			AsyncTaskStarted();
			SendAsyncRequest("GET", "/_local/" + RemoteCheckpointDocID(), null, new _CBLRemoteRequestCompletionBlock_453
				(this, localLastSequence));
		}

		private sealed class _CBLRemoteRequestCompletionBlock_453 : CBLRemoteRequestCompletionBlock
		{
			public _CBLRemoteRequestCompletionBlock_453(CBLReplicator _enclosing, string localLastSequence
				)
			{
				this._enclosing = _enclosing;
		...
		public virtual void SaveLastSequence()
		{
			...
			SendAsyncRequest("PUT", "/_local/" + remoteCheckpointDocID, body, new _CBLRemoteRequestCompletionBlock_509
				(this, body));
		...
		private sealed class _CBLRemoteRequestCompletionBlock_509 : CBLRemoteRequestCompletionBlock
		{
			public _CBLRemoteRequestCompletionBlock_509(CBLReplicator _enclosing, IDictionary
				<string, object> body)
			{
				this._enclosing = _enclosing;
				this.body = body;
