...
namespace Couchbase
{
	public class CBLManager
	{
		...
		/// <summary>Releases all resources used by the CBLManager instance and closes all its databases.
		/// 	</summary>
		/// <remarks>Releases all resources used by the CBLManager instance and closes all its databases.
		/// 	</remarks>
		/// <returns></returns>
		public virtual void Close()
		{
			...
			server.Close();
			IList<CBLDatabase> databases = (IList<CBLDatabase>)server.AllOpenDatabases();
			foreach (CBLDatabase database in databases)
			{
				IList<CBLReplicator> replicators = database.GetAllReplications();
				foreach (CBLReplicator replicator in replicators)
				{
					replicator.Stop();
				}
				database.Close();
