...
namespace Couchbase.Router
{
	public class CBLRouter : Observer
	{
		...
		public virtual void Start()
		{
			...
			CBLStatus status_1 = null;
			try
			{
				MethodInfo m = this.GetType().GetMethod(message, typeof(CBLDatabase), typeof(string
					), typeof(string));
			...
			catch (NoSuchMethodException)
			{
				try
				{
					...
					MethodInfo m = this.GetType().GetMethod("do_UNKNOWN", typeof(CBLDatabase), typeof(
						string), typeof(string));
					status_1 = new CBLStatus(CBLStatus.BadRequest);
				}
				catch (Exception e)
				{
			...
			catch (Exception e)
			{
				...
				connection.SetResponseBody(new CBLBody(result));
				if (e is CBLiteException)
				{
					status_1 = ((CBLiteException)e).GetCBLStatus();
				}
				else
				{
					status_1 = new CBLStatus(CBLStatus.NotFound);
			 ...
			 _attachmentName)
		{
			...
			{
				IList<CBLReplicator> activeReplicators = db.GetAllReplications();
		...
		private void ConvertCBLQueryRowsToMaps(IDictionary<string, object> allDocsResult)
		{
			IList<IDictionary<string, object>> rowsAsMaps = new AList<IDictionary<string, object
				>>();
			IList<CBLQueryRow> rows = (IList<CBLQueryRow>)allDocsResult.Get("rows");
			...
			, string _attachmentName)
		{
			...
			foreach (CBLRevisionInternal rev_1 in revs)
			{
		...
		public virtual IDictionary<string, object> ChangesDictForRevision(CBLRevisionInternal
			 rev)
		{
		...
		public virtual IDictionary<string, object> ResponseBodyForChanges(IList<CBLRevisionInternal
			> changes, long since)
		{
			IList<IDictionary<string, object>> results = new AList<IDictionary<string, object
				>>();
			foreach (CBLRevisionInternal rev in changes)
			{
		...
		public virtual IDictionary<string, object> ResponseBodyForChangesWithConflicts(IList
			<CBLRevisionInternal> changes, long since)
		{
			...
			entries.Sort(new _IComparer_1006());
		...
		private sealed class _IComparer_1006 : IComparer<IDictionary<string, object>>
		{
			public _IComparer_1006()
			{
			}
		...
		public virtual void SendContinuousChange(CBLRevisionInternal rev)
		{
			IDictionary<string, object> changeDict = ChangesDictForRevision(rev);
			try
			{
		...
		public virtual void Update(Observable observable, object changeObject)
		{
			if (observable == db)
			{
				...
				if (longpoll)
				{
					...
					SendResponse();
					IList<CBLRevisionInternal> revs = new AList<CBLRevisionInternal>();
					revs.AddItem(rev);
		...
		public virtual string SetResponseEtag(CBLRevisionInternal rev)
		{
			...
			try
			{
				...
				else
				{
					...
					if (openRevsParam.Equals("all"))
					{
						...
						foreach (CBLRevisionInternal rev in allRevs)
						{
							try
							{
							...
							{
								if (e.GetCBLStatus().GetCode() != CBLStatus.InternalServerError)
								{
		...
		/// 	</summary>
		public virtual CBLRevisionInternal Update(CBLDatabase _db, string docID, CBLBody 
			body, bool deleting, bool allowConflict, CBLStatus outStatus)
		{
		...
		/// <exception cref="Couchbase.CBLiteException"></exception>
		public virtual void Do_PUT_Document(CBLDatabase _db, string docID, string _attachmentName
			)
		{
			IDictionary<string, object> bodyDict = GetBodyAsDictionary();
			if (bodyDict == null)
			 ...
			 contentStream)
		{
			...
			CBLRevisionInternal rev = db.UpdateAttachment(attachment, contentStream, connection
				.GetRequestProperty("content-type"), docID, revID, status);
		...
		/// <exception cref="Couchbase.CBLiteException"></exception>
		public virtual void Do_PUT_Attachment(CBLDatabase _db, string docID, string _attachmentName
			)
		{
		...
		/// <exception cref="Couchbase.CBLiteException"></exception>
		public virtual void Do_DELETE_Attachment(CBLDatabase _db, string docID, string _attachmentName
			)
		{
			...
			object> keys)
		{
			...
			if (view == null || view.GetMap() == null)
			{
				...
				CBLRevisionInternal rev = db.GetDocumentWithIDAndRev(string.Format("_design/%s", 
					designDoc), null, EnumSet.NoneOf<CBLDatabase.TDContentOptions>());
				if (rev == null)
				{
					return new CBLStatus(CBLStatus.NotFound);
				}
			...
			IList<CBLQueryRow> queryRows = view.QueryWithOptions(options);
			IList<IDictionary<string, object>> rows = new AList<IDictionary<string, object>>(
				);
			foreach (CBLQueryRow queryRow in queryRows)
			{
