...
namespace Couchbase
{
	...
	public class CBLServer
	{
		...
		{
			FilePath[] files = directory.ListFiles(new _FilenameFilter_88());
		...
		private sealed class _FilenameFilter_88 : FilenameFilter
		{
			public _FilenameFilter_88()
			{
			}
		...
		{
			string[] databaseFiles = directory.List(new _FilenameFilter_172());
		...
		private sealed class _FilenameFilter_172 : FilenameFilter
		{
			public _FilenameFilter_172()
			{
			}
		...
		{
			Future<object> closeFuture = workExecutor.Submit(new _Runnable_197(this));
			try
			{
				closeFuture.Get();
				...
				workExecutor.Schedule(new _Runnable_215(this), 60, TimeUnit.Seconds);
			}
			catch (Exception e)
			{
		...
		{
			public _Runnable_197(CBLServer _enclosing)
			{
		...
		private sealed class _Runnable_215 : Runnable
		{
			public _Runnable_215(CBLServer _enclosing)
			{
				this._enclosing = _enclosing;
			}
